...
public interface BusinessPartnerRepository extends CrudRepository<BusinessPartner, UUID> {
    ...
    @Query("SELECT * FROM business_partner WHERE tenant_id = :tenantId AND code = :code")
    Optional<BusinessPartner> findByCode(@Param("tenantId") UUID tenantId, @Param("code") String code);
}
